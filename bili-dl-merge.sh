	...
	if [ $return_value -eq 0 ];then #选择确定
		if [[ -d "$file_select" ]];then #选择的是目录
			...
			if [[ $file_select == *$file_format ]];then #选择的文件是指定格式
				...
				dialog --clear --title "文件管理" --msgbox "文件格式错误,请选择以m4s后缀的文件" 25 60
				filemanager
			fi
		else 
...
function start_option()
{
	...
	start_option_=$(dialog --clear --title "启动选项" --menu "请选择要进行的操作\n第一次使用推荐设置启动路径\n启动文件管理后寻找后缀为\"m4s\"的文件即可开始转换\n\n当前配置文件状态:"$config_status"\n启动路径:\n"$_setup_path"\n保存路径:\n"$_output_path"\n\n使用方向键,Tab键,回车键进行选择" 25 60 10 \
			"1" "启动文件管理" \
			"2" "设置启动路径" \
			"3" "更新脚本" \
			"4" "退出" \
			3>&1 1>&2 2>&3)
	if [ $? = 0 ];then
		if [ $start_option_ = "1" ];then #选择启动文件管理
			if [ -e .bili_dl_merge.ini ];then
				final_setup_path=$(cat "$script_start_path"/.bili_dl_merge.ini | awk 'NR==3' )
				cd $final_setup_path
			fi
			filemanager
		elif [ $start_option_ = "2" ];then #选择设置启动路径
			setup_path
		elif [ $start_option_ = "3" ];then
			update_option
		elif [ $start_option_ = "4" ];then
			exit
		fi
	fi
	exit
...
function setup_path()
{
	...
	#设置启动路径
	final_setup_path=$(dialog --clear --title "启动路径设置" --inputbox "请输入哔哩哔哩视频下载的目录\n哔哩哔哩目录在Android/data\n目录的格式例子：\n/storage/emulated/0/Android/data/tv.danmaku.bili/download" 25 60 3>&1 1>&2 2>&3)
	setup_path_status=$?
